...
class run_tiago:
    ...
    def __init__(self, mode=1):
        ...
        self.aruco_pos = [1.925, 1.6]   # place to check aruco
        ...
        self.torso_height_aruco = 0.35
        self.torso_height_dropoff_table = 0.05
        self.head_rot_table = [0.00, -0.51]
    ...
    def run(self):
        ...
        if self.mode == 0: # do nothing
            ...
            # rospy.loginfo("trying to get object with id 10")
            ...
            # # Move to table
            # self.move_to([self.free_space[0], self.free_space[1], 0], 2)     # turn right
            # self.move_to([self.item_pos[0], self.free_space[1], 0], 0)      # move forward
            # self.move_to([self.item_pos[0], self.free_space[1], -90], 2)    # turn to face table
            # self.move_to([self.item_pos[0], self.item_pos[1], -90], 0)     # move to in front of table
            # rospy.loginfo("Arrived at Table")
            ...
            # rospy.loginfo("moving back to table center")
    ...
    def calc_coords_of_object(pose):
        ...
        y_to_move = self.aruco_pos[1] - center_to_palm + x_from_base
        x_to_move = self.aruco_pos[0] - body_to_midline + y_from_base
